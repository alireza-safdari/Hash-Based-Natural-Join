...
uint32_t getHashCode(uint32_t input, uint32_t hashDevider) // max must not be included in range
{
    return input % hashDevider;
}
...
template <class tuppleType>
uint32_t hashRelation(tuppleType& relationOnDisk, tuppleType& relationOnMem, tuppleType hashOnDisk[], tuppleType hashOnMem[], uint32_t hashDevider)
{
    ...
    while (!hashingEnded)
    {
        ...
        {
            uint32_t hashCode = getHashCode(relationOnMem[i].B, hashDevider);
...
int main()
{
    ...
    std::cout << "Relation S generated with 5000 tupples.\n";

    generateRRelationType1(relationR, relationS, 1000);
    std::cout << "Relation R is generated with 1000 tuples.\n";


    uint32_t nSTupplesHashed = hashRelation(relationS, sMemory, relationSHash, sHashMemory, N_HASH_BUCKETS);
    std::cout << "\nHashed S, " << nSTupplesHashed << " tuples were hashed.\n";
    for (uint32_t i = 0; i < N_HASH_BUCKETS; i++)
    {
        std::cout << "Bucket[" << i << "]: remainder is " << i << " when devided by "
            << N_HASH_BUCKETS << " (number of buckets), and has " << relationSHash[i].size() << " tuples\n";
    }


    uint32_t nRTupplesHashed = hashRelation(relationR, rMemory, relationRHash, rHashMemory, N_HASH_BUCKETS);
    std::cout << "\nHashed R, " << nRTupplesHashed << " tuples were hashed.\n";
    for (uint32_t i = 0; i < N_HASH_BUCKETS; i++)
    {
        std::cout << "Bucket[" << i << "]: remainder is " << i << " when devided by "
            << N_HASH_BUCKETS << " (number of buckets), and has " << relationRHash[i].size() << " tuples\n";
    ...
    nSTupplesHashed = hashRelation(relationS, sMemory, relationSHash, sHashMemory, N_HASH_BUCKETS);
    std::cout << "\nHashed S, " << nSTupplesHashed << " tuples were hashed.\n";
    for (uint32_t i = 0; i < N_HASH_BUCKETS; i++)
    {
        std::cout << "Bucket[" << i << "]: remainder is " << i << " when devided by "
    ...
    nRTupplesHashed = hashRelation(relationR, rMemory, relationRHash, rHashMemory, N_HASH_BUCKETS);
    std::cout << "\nHashed R, " << nRTupplesHashed << " tuples were hashed.\n";
    for (uint32_t i = 0; i < N_HASH_BUCKETS; i++)
    {
        std::cout << "Bucket[" << i << "]: remainder is " << i << " when devided by "
